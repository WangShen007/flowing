...
async def _probe_cli_state(user_id: str) -> SetupCLIState:
    from app.core.feishu_tokens import enterprise_configured

    if enterprise_configured():
        state = await LarkCLISkill()._probe_cli_state(user_id)
        return SetupCLIState(
            user_id=user_id, profile=state.profile, installed=state.installed,
            configured=state.configured, authenticated=state.authenticated,
            install_info=state.install_info, config_info=state.config_info, auth_info=state.auth_info,
        )
...
async def stream_lark_setup(
    ...
    if enterprise_configured():
        raise HTTPException(status_code=409, detail="请使用企业飞书授权入口连接账号")
