...
from app.core.feishu_permissions import scenario_access
...
def _enterprise_scenario_payload(template: dict, role: str) -> dict:
    access = scenario_access(str(template["id"]), role)
    return {
        **template,
        "enterprise_ready": access.ready,
        "enterprise_reason": access.reason,
        "required_capabilities": list(access.capabilities),
    }


@router.get("/scenarios")
async def list_scenarios(account: AccountInfo = Depends(get_current_account)) -> dict:
    builtins = [*SCENARIO_TEMPLATES]
    if feishu_tokens.enterprise_configured():
        builtins = [_enterprise_scenario_payload(template, account.role) for template in builtins]
    return {"code": 0, "data": [*builtins, *list_accessible_templates(account)]}
...
async def render_scenario(
    ...
    try:
        user_template = get_template_for_render(request.template_id, account)
        if user_template:
            ...
            if template:
                if feishu_tokens.enterprise_configured():
                    access = scenario_access(request.template_id, account.role)
                    if not access.ready:
