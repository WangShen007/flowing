...
class LocalSessionStore:
    ...
    def _safe_user_id(user_id: str) -> str:
        # This is a parameterized SQLite identity, not a filesystem path.
        # Lossy sanitization aliases distinct authenticated accounts (a.b/a-b).
        # Keep the canonical account verbatim; callers must supply an identity.
        if not isinstance(user_id, str) or not user_id or len(user_id) > 200:
            raise ValueError("A non-empty account identity is required")
        return user_id
    ...
    def get_or_create(self, user_id: str, session_id: str = "", *, include_messages: bool = True) -> dict[str, Any]:
        ...
        return {
            ...
            "messages": self._messages_for(resolved_user, resolved_session) if include_messages else [],
        }
    ...
    def append_message(
        ...
        resolved_user = self._safe_user_id(user_id)
        session = self.get_or_create(resolved_user, session_id, include_messages=False)
        ...
        if role == "user" and title in {"", "New chat"}:
            ...
            store.execute(
                "UPDATE chat_sessions SET title = CASE WHEN title IN ('', 'New chat') THEN ? ELSE title END, "
                "updated_at = ? WHERE user_id = ? AND session_id = ?",
