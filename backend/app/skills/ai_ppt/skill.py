...
from app.api.routes.ai_ppt import (
    ai_ppt_base_dir,
    register_ai_ppt_file,
    resolve_ai_ppt_file,
    resolve_ai_ppt_template,
)
...
class AIPPTSkill(BaseSkill):
    ...
    async def execute_stream(self, context: SkillContext, **kwargs: Any) -> AsyncGenerator[dict[str, Any], None]:
        ...
        previous_path = (
            resolve_ai_ppt_file(previous.get("filename"), context.user_id)
            if previous and previous.get("filename")
            else None
        )
        previous_version = int(previous.get("version", 0)) if previous else 0

        source_path = resolve_ai_ppt_file(source_ppt_id, context.user_id) if source_ppt_id else None
        template_path = resolve_ai_ppt_template(template_id, context.user_id)
        if source_ppt_id and source_path is None:
            message = "无法读取指定的源 PPT：文件不存在、尚未绑定账号，或不属于当前账号。"
            yield {"type": "content", "content": message}
            yield {"type": "metadata", "data": {"ai_ppt_error": message, "ai_ppt_forbidden": True}}
            return
        if template_id and template_path is None:
            message = "无法读取指定的 PPT 模板：模板不存在、尚未绑定账号，或不属于当前账号。"
            yield {"type": "content", "content": message}
            yield {"type": "metadata", "data": {"ai_ppt_error": message, "ai_ppt_forbidden": True}}
            return
...
{{
  ...
  "slides": [
    ...
    def _render_deck(
        ...
        register_ai_ppt_file(filename, owner_account, "generated")
    ...
    def _build_response_message(self, result: dict[str, Any], action: str, style: str, followup_text: str = "") -> str:
        ...
        return (
            ...
            f"- 风格：{style}\n"
            f"{source_line}"
            f"- 文件：{result['filename']}（请使用下方 PPT 卡片的“下载 PPTX”按钮）\n\n"
            f"页面结构：\n{slide_titles}\n\n"
            f"{followup_line}"
